...
def index():
    ...
    stocks = db.execute("SELECT symbol, name, price, SUM(shares) as totShares FROM transactions WHERE user_id = ? GROUP by symbol", user_id)
    cash = db.execute("SELECT cash FROM users WHERE id = ?", user_id)[0]["cash"]

    tot = cash

    for stock in stocks:
        tot += stock["price"] * stock["totShares"]

    return render_template("index.html", stocks=stocks, cash=cash, usd=usd, tot=tot)
...
def buy():
    ...
    if request.method == "POST":
        ...
        name = stock["name"]
        price = stock["price"]
        price_tot = price*shares

        if cash < price_tot:
            return apology("Need more money")

        else:
            db.execute("UPDATE users SET cash = ? WHERE id = ?", cash - price_tot, user_id)
...
def register():
    ...
    if request.method == "POST":
        ...
        if not username:
            return apology('Username reqiured')
...
def sell():
    ...
    if request.method == "POST":
        ...
        price_t = price*shares

        owned = db.execute("SELECT shares FROM transactions WHERE user_id = ? AND symbol = ? GROUP BY symbol", user_id, symbol)[0]["shares"]

        # if owned < shares:
        #     return apology("Not enough shares")

        curr_cash = db.execute("SELECT cash FROM users WHERE id = ?", user_id)[0]["cash"]

        db.execute("UPDATE users SET cash = ? WHERE id = ?", curr_cash + price_t, user_id)
